...
void test_all_supplier_functions() {
  ...
  munit_assert_int(test_start_suppliers_operation(6), ==, 6);
  munit_assert_int(test_start_suppliers_operation(7), ==, 7);
  ...
  //test_view_suppliers_id
  int supplier[10] = {1,3,2,5,6,7,8,9,4,10};
  munit_assert_int(test_view_suppliers_id(active1,supplier, 1), ==, 1);
  munit_assert_int(test_view_suppliers_id(active1,supplier, 7), ==, 0);
  munit_assert_int(test_view_suppliers_id(active3,supplier, 7), ==, 0);
  //test_view_suppliers_name
  char nome[100] = "Carlos festas";
  munit_assert_int(test_view_suppliers_name(active1,nome), ==, 1);
  strcpy(nome, "Festa");
  munit_assert_int(test_view_suppliers_name(active1,nome), ==, 0);
  munit_assert_int(test_view_suppliers_name(active3,nome), ==, 0);
}
...
int test_view_suppliers_id(int active[10], int supplier[10], int codigo) {
  int i = 0;
  while (i < 10) {
    if (active[i] && codigo == supplier[i]) {
      return 1;
    }
    i++;
  }
...
int test_view_suppliers_name(int active[10], char nome[100]) {
  char nomeS[100] = "Carlos festas";
  int i = 0, find = 0;
  while (i < 10) {
    if (active[i] && strcmp(nome, nomeS) == 0) {
      return 1;
      find = 1;
    }
    i++;
