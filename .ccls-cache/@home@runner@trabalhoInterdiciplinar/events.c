...
int check_event_day_time(FILE *f, char data[100], char horaInicio[100]){
...
void include_events(FILE *f) {
  ...
  if(posicaoDesactive == -1){
    ...
    if (posicao == -1) {
      ...
      scanf(" %[^\n]", e.horarioFim);
      // if((atoi(e.horarioFim) - atoi(e.horarioInicio)) > 4){
      //   printf("A festa não pode ter mais de 4 horas\n\n");
      //   return;
      // }
      // off-line
      if(!strcmp(e.diaSemana, dia))
      if(verificar_duracao(e.horarioInicio, e.horarioFim)){
        printf("A festa não pode ter mais de 4 horas\n\n");
        return;
      }
      if(!check_event_day_time(f, e.data, e.horarioInicio)){
    ...
    if(posicao == -1){
      ...
      // if(verificar_duracao(e.horarioInicio, e.horarioFim)){
      //   printf("A festa não pode durar mais de 4 horas!");
      //   return;
      // }
      if(!check_event_day_time(f, e.data, e.horarioInicio)){
        printf("Festa já cadastrada nesse dia e horário\n\n");
