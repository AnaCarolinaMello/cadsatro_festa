...
void start_supplier_operation() {
  ...
  do {
    ...
    printf("5 - Pesquisar fornecedor pelo código\n");
    printf("6 - Pesquisar fornecedor pelo nome\n");
...
void view_suppliers_name(FILE *f) {
  supplier s;
  char nome[100];
  int find = 0;
  ...
  fread(&s, sizeof(s), 1, f);
  while (!feof(f)) {
    if (s.active && strcmp(s.nome, nome) == 0) {
      printf("Código.............: %d \n", s.codigo);
      printf("Nome...............: %s \n", s.nome);
      printf("Telefone...........: %s\n", s.telefone);
      printf("E-mail.............: %s\n", s.email);
  ...
  if(!find){
    printf("Fornecedor %s não encontrado\n\n", nome);
  }
}